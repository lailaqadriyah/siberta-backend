...
- `POST /api/submissions/{id}/upload` — upload files
- `POST /api/submissions/{id}/reviews` — dosen review (decision & comment)
- `POST /api/admin/validate/{submissionId}` — admin final validation
- `POST /api/admin/sync-ml` — trigger SQL → .pkl sync job

See `openapi.yaml` for full request/response contract and schemas.

Security: APIs use `Authorization: Bearer <token>` (JWT). Implement role-based middleware (mahasiswa/dosen/admin).
